...
const availableResources = async(req,res) => {
    try
    {
        const result = await db.query(query.availableResources);
        if(result.rowCount == 0)
        {
            res.status(204).json({success : true , message : "No resources Available"});
        }
        else
        {
