...
const query = require(path.resolve(__dirname , '../queries/discussion_box.js'));

const getByTagsLatest = async(req,res) => {
    try
    {
        const req_data = req.query;
        const result = await db.query(query.getByTagsLatest , [req_data.tag]);
        if(result.rowCount == 0)
        {
            res.status(204).json({success: true , message: "No Posts"});
        }
        else
        {
...
module.exports = getByTagsLatest;