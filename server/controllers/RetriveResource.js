...
const query = require(path.resolve(__dirname, '../queries/resources_table.js'));

const deleteMember = async (req, res) => {
    //console.log("reached here");
    try {
        const data = req.body;
        const result = await db.query(query.retriveResource , [data.resource_assignment_id]);
        if(result.rowCount == 0)
        {
            res.status(404).json({success : false , message : "No resource assignment found"});
        }
        else
        {
            res.status(200).json({success : true , message : "resource retrived"});
        }
    }
    catch (error) { 
